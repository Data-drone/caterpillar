...
def test_index_open(index_dir):
    with open(os.path.abspath('caterpillar/test_resources/alice_test_data.txt'), 'r') as f:
        data = f.read()
        storage_writer = SqliteWriter(index_dir, create=True)
        storage_reader = SqliteReader(index_dir)
        writer = IndexWriter(storage_writer)
        reader = IndexReader(storage_reader)
        ...
        with writer:
            writer.add_fields(**schema_spec)

        with reader:
            assert reader.revision == (0, 0, 0, 0)

        with writer:
            writer.add_document(dict(text1=data, text2=data, document='alice.txt', flag=True), frame_size=2)
...
def test_index_alice(index_dir):
    ...
    with open(os.path.abspath('caterpillar/test_resources/alice_test_data.txt'), 'r') as f:
        ...
        with writer:
            writer.add_fields(
                text=dict(type='test_text'),
                document=dict(type='test_text', indexed=False),
                blank=dict(type='numeric', indexed=True),
                ref=dict(type='id', indexed=True)
            )
            writer.add_document(dict(text=data, document='alice.txt', blank=None, ref=123), frame_size=2)

        doc_id = writer.last_committed_documents[0]

        reader = IndexReader(SqliteReader(index_dir))
        ...
        with writer:
            writer.add_fields(field1=dict(type='text'), field2=dict(type='numeric', indexed=True))
        ...
        with writer:
            with pytest.raises(TypeError):
                writer.add_document(dict(text=False, document='alice'), frame_size=0)

        # Test frame size = 0 (whole document)
        with writer:
            writer.add_document(dict(text=data, document='alice'), frame_size=0)
            writer.add_document(dict(text=unicode("unicode data"), document='test'), frame_size=0)
...
def test_index_alice_attributes(index_dir):
    ...
    with open(os.path.abspath('caterpillar/test_resources/alice_test_data.txt'), 'r') as f:
        ...
        with writer:
            writer.add_fields(
                text1=dict(type='test_text'),
                text2=dict(type='text'),
                document=dict(type='test_text', indexed=False),
                blank=dict(type='numeric', indexed=True),
                ref=dict(type='id', indexed=True)
            )
...
def test_index_writer_rollback(index_dir):
    with open(os.path.abspath('caterpillar/test_resources/alice_test_data.txt'), 'r') as f:
        ...
        writer.begin()
        writer.add_fields(text={'type': 'test_text'})
        writer.add_document(dict(text=data))
        writer.rollback()
...
def test_alice_case_folding(index_dir):
    """Test case normalisation estimation. """
    with open(os.path.abspath('caterpillar/test_resources/alice.txt'), 'r') as f:
        f.seek(0)
        data = f.read()
...
def test_index_utf8(index_dir):
    with open(os.path.abspath('caterpillar/test_resources/mt_warning_utf8.txt'), 'r') as f:
        ...
        with writer:
            writer.add_fields(text=dict(type='test_text'), document=dict(type='test_text', indexed=False))
            writer.add_document(dict(text=data, document='mt_warning_utf8.txt'), frame_size=2)
...
def test_index_latin1(index_dir):
    with open(os.path.abspath('caterpillar/test_resources/mt_warning_latin1.txt'), 'r') as f:
        ...
        with writer:
            writer.add_fields(text=dict(type='test_text'), document=dict(type='test_text', indexed=False))
            writer.add_document(dict(text=data, document='mt_warning_latin1.txt'), frame_size=2, encoding='latin1')

            with pytest.raises(IndexError):
                # Bad encoding
                writer.add_document(dict(text=data, document='mt_warning_latin1.txt'), frame_size=2)

            # Ignore bad encoding errors
            writer.add_document(
                dict(text=data, document='mt_warning_latin1.txt'),
                frame_size=2, encoding_errors='ignore'
            )
...
def test_index_encoding(index_dir):
    ...
    with writer:
        writer.add_fields(text=dict(type='text'))
        writer.add_document(dict(text=u'This is a unicode string to test our field decoding.'), frame_size=2)

        with open(os.path.abspath('caterpillar/test_resources/mt_warning_utf8.txt'), 'r') as f:
            data = f.read()
        with pytest.raises(IndexError):
            writer.add_document(dict(text=data), frame_size=2, encoding='ascii')
...
def test_index_state(index_dir):
    with open(os.path.abspath('caterpillar/test_resources/detractors.csv'), 'rbU') as f:
        ...
        with IndexWriter(SqliteWriter(index_dir, create=True)) as writer:
            writer.add_fields(text=dict(type='text'))
            row = csv_reader.next()
            writer.add_document(dict(text=row[0]))
        ...
        added_docs = writer.last_committed_documents
        ...
        with writer:
            writer.delete_document(added_docs[0])
...
def test_index_reader_writer_isolation(index_dir):
    ...
    with open(os.path.abspath('caterpillar/test_resources/alice_test_data.txt'), 'r') as f:
        ...
        with IndexReader(SqliteReader(index_dir)) as reader1:
...
def test_index_multi_document_delete(index_dir):
    ...
    with open(os.path.abspath('caterpillar/test_resources/alice_test_data.txt'), 'r') as f:
        ...
        doc_ids = writer.last_committed_documents
...
def test_metadata_only_retrieval_deletion(index_dir):
    ...
    with IndexWriter(SqliteWriter(index_dir, create=True)) as writer:
        writer.add_fields(text=dict(type='text'), num=dict(type='numeric', indexed=True))
        writer.add_document(dict(num=1))
    ...
    with IndexReader(SqliteReader(index_dir)) as reader:
        assert reader.get_document_count() == 1
        assert reader.get_frame_count('text') == 0
        doc_results = reader.filter(metadata={'num': {'=': 1}}, return_documents=True)
        assert len(doc_results) == 1
        frame_results = reader.filter(metadata={'num': {'=': 1}}, return_documents=False)
...
def test_field_names(index_dir):
    ...
    schema = {"Don't Like": {'type': 'text'}, "Would Like": {'type': 'text'}}
    document1 = {"Don't Like": 'The scenery was unpleasant.', "Would Like": 'More cats.'}

    with IndexWriter(SqliteWriter(index_dir, create=True)) as writer:
        writer.add_fields(**schema)
...
def _acquire_write_single_doc(index_dir):
    ...
    writer = IndexWriter(SqliteWriter(index_dir))
    with writer:
